...
void TimeAveraging::post_advance_work()
{
    ...
    amrex::Real on_interval = (((cur_time - m_start_time) + t_tol) / 
                                  m_time_interval - 
                              std::floor(((cur_time - m_start_time) + t_tol) /
                                  m_time_interval)) -
                              (cur_dt / m_time_interval);
    
    const bool do_phase_avg = ((m_time_interval < 0.0) || (on_interval < 0.0));

    const bool do_avg = (((cur_time >= m_start_time) && 
                          (cur_time < m_stop_time)) &&
                          do_phase_avg);

    if (do_avg) {
        amrex::Print() << "Averaging " << m_label << "..." << std::endl;
    }
